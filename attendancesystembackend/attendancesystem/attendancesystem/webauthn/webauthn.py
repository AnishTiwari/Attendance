...
class WebAuthnMakeCredentialOptions(object):
    ...
    def __init__(
        ...
        if attestation not in self._attestation_forms:
            raise ValueError(
                "Attestation must be a string and one of "
...
class WebAuthnAssertionOptions(object):
    def __init__(
        self, webauthn_user, challenge, timeout=60000, userVerification="discouraged"
    ):
...
class WebAuthnUser(object):
    ...
    def __str__(self):
        return "{} ({}, {}, {})".format(
            self.user_id, self.username, self.display_name, self.sign_count
        )
        ...
        try:
            ...
            if registration_client_extensions:
                ...
                if not _verify_client_extensions(
                    rce, self.expected_registration_client_extensions
                ):
                    raise RegistrationRejectedException(
            ...
            trust_anchors = _get_trust_anchors(
                attestation_type, fmt, self.trust_anchor_dir
            )
            ...
            #       database. See `flask_demo/__init__.py`.
...
class WebAuthnAssertionResponse(object):
    ...
    def verify(self):
        try:
            ...
            if (
                isinstance(self.webauthn_user.sign_count, int)
                and self.webauthn_user.sign_count < 0
            ) or not isinstance(self.webauthn_user.sign_count, int):
